...
const MoveBox = ({ initPosition }: { initPosition: number }) => {
  ...
  // 의존성 배열 부분을 아예 제외 : 컴포넌트가 업데이트 될 때마다 실행
  // [] : 최초 한 번만 실행 되고 더 이상 실행되지 않음
  ...
  useEffect(() => {
    console.log("useEffext 실행 ==> 3. 컴포넌트 마운트");
    return () => {
      console.log("useEffext 실행 ==> 4. 컴포넌트 언마운트");
    };
  });
